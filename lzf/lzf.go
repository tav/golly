...
// Package lzf implements the VERY_FAST variant of the LZF compression
// algorithm by Marc Alexander Lehmann.
//
// Although not as fast as libLZF which is extremely optimised thanks to pointer
// arithmetic, this still works a lot faster than many other compression
// algorithms. The compression ratio is also, surprisingly, often better for
// formats like JSON.
package lzf

const (
	// The hashTableFactor can be set to anything from 13 all the way up to 23.
