...
// Package oauth implements OAuth compatible with most web services.
package oauth
...
// Token contains an end-user's tokens. This is the data you must store to
...
// AuthCodeURL returns a URL that the end-user should be redirected to, so that
// they may obtain an authorization code.
...
// ExchangeAuthorizationCode takes a code and gets access Token from the remote
// server. http://tools.ietf.org/html/draft-ietf-oauth-v2-23#section-4.2.1
...
// ExchangeClientCredentials uses the client credentials and gets an access
...
// AlternateTransport implements http.RoundTripper. It sets an additional header
// value for OAuth 2 providers that detect the Accept header.
